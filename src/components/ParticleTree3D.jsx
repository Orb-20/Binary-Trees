...
function generateHeap({ levels = 4, width = 4.5, height = 4 }) { return generateCompleteBinaryTree({ levels, width, height }); }
function generateTrie({ height = 4 }) {
  const nodeCenters = [ { x: 0, y: 1.8, z:0, children: [1, 2, 3] }, { x: -2.0, y: 0.6, z:0, children: [4, 5] }, { x: 0, y: 0.6, z:0, children: [6] }, { x: 2.0, y: 0.6, z:0, children: [7, 8] }, { x: -2.5, y: -0.6, z:0, children: [] }, { x: -1.5, y: -0.6, z:0, children: [] }, { x: 0, y: -0.6, z:0, children: [9] }, { x: 1.5, y: -0.6, z:0, children: [] }, { x: 2.5, y: -0.6, z:0, children: [] }, { x: 0, y: -1.8, z:0, children: [] }, ];
...
function generateGraph({ height = 4 }) {
  const nodeCenters = [ { x: -2, y: 1.5, z:0, children: [1, 3] }, { x: 0, y: 1.5, z:0, children: [2, 4] }, { x: 2, y: 1.5, z:0, children: [5] }, { x: -2, y: -0.5, z:0, children: [4] }, { x: 0, y: -0.5, z:0, children: [5] }, { x: 2, y: -0.5, z:0, children: [] }, ];
  nodeCenters.forEach(n => { n.y -= yOffset; });
  return createParticleTargets(nodeCenters);
}
...
function generateStarGraph({ count = 8, radius = 2.5 }) {
    ...
    for (let i = 1; i <= count; i++) {
        ...
        nodeCenters.push({ x: radius * Math.cos(angle), y: radius * Math.sin(angle), z: 0, children: [] });
    }
    nodeCenters.forEach(n => { n.y -= yOffset; });
    return createParticleTargets(nodeCenters);
}
function generateRingGraph({ count = 10, radius = 2 }) {
    ...
    for (let i = 0; i < count; i++) {
        const angle = (i / count) * Math.PI * 2;
        nodeCenters.push({ x: radius * Math.cos(angle), y: radius * Math.sin(angle), z: 0, children: [(i + 1) % count] });
    }
    nodeCenters.forEach(n => { n.y -= yOffset; });
    return createParticleTargets(nodeCenters);
}
...
function generateFibonacciTree(n) {
    if (n <= 1) return [{ x: 0, y: 0, z: 0, children: [] }];
    const leftSubtree = generateFibonacciTree(n - 1);
    const rightSubtree = generateFibonacciTree(n - 2);
    const root = { x: 0, y: 1.5, z: 0, children: [] };
    const offset = 2.5;
    leftSubtree.forEach(node => { node.x -= offset / 2; node.y -= 1.5; });
    rightSubtree.forEach(node => { node.x += offset / 2; node.y -= 1.5; });
...
function ParticleFieldInstanced({ size = 0.035 }) {
  const meshRef = useRef();
  const shapes = useMemo(() => [ generatePerfectBinaryTree({}), generateCompleteBinaryTree({}), generateHeap({}), generateTrie({}), generateSkewedTree({}), generateGraph({}), generateTernaryTree({}), generateStarGraph({}), generateRingGraph({}), generateGridGraph({}), createParticleTargets(generateFibonacciTree(5).map(n => ({...n, y: n.y - yOffset}))), ], []);
  ...
  useEffect(() => { for (let i = 0; i < totalParticles; i++) seedRef.current[i] = Math.random() * 1000; }, [totalParticles]);
  ...
  useFrame(({ clock }) => {
    ...
    for (let i = 0; i < totalParticles; i++) {
      ...
      if (!isVisibleInCurrent && !isVisibleInNext) scaleFactor = 0;
      ...
      if (isLinkInNextShape) {
        ...
        if (normalizedPathPosition > drawHead) scaleFactor = 0;
      }
  ...
  const material = useMemo(() => new THREE.MeshBasicMaterial({ color: new THREE.Color("#0FA683") }), []); // Jade
