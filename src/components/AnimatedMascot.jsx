...
import { MASCOT_META } from '../mascot.config'; // Import from the new config file
  ...
  bolt: (refs) => (
     <svg viewBox="0 0 200 200" role="img" aria-label="Bolt mascot">
        <g ref={refs.container}>
            ...
            <rect ref={refs.leftEye} x="88" y="65" width="8" height="8" rx="2" fill={MASCOT_META.bolt.accent} />
            <rect ref={refs.rightEye} x="104" y="65" width="8" height="8" rx="2" fill={MASCOT_META.bolt.accent} />
  ...
  sage: (refs) => (
    <svg viewBox="0 0 200 200" role="img" aria-label="Sage mascot">
        <g ref={refs.container}>
            ...
            <circle ref={refs.leftEye} cx="90" cy="95" r="5" fill="#333" />
            <circle ref={refs.rightEye} cx="110" cy="95" r="5" fill="#333" />
...
const runAnimation = (reaction, refs) => {
    anime.remove([refs.container.current, refs.leftArm.current, refs.rightArm.current, refs.mouth.current]);
    anime({ targets: refs.container.current, translateY: [-2, 0, -2], easing: 'easeInOutSine', duration: 2500, loop: true });
    const mouthSpeaking = () => {
        if(refs.mouth.current) {
            anime({ targets: refs.mouth.current, scaleY: [1, 0.4, 1, 0.6, 1], duration: 700, loop: false, easing: 'easeInOutSine' });
        }
    };
    switch (reaction) {
        case 'greet':
